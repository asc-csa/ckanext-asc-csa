...
from six.moves.urllib.parse import urlencode, quote, unquote, urlparse, urlunparse
    ...
    def __getitem__(self, key):
        try:
            ...
            raise ckan.exceptions.HelperError(
                "Helper '{key}' has not been defined.".format(key=key)
            )
        return value
...
def redirect_to(*args, **kw):
    ...
    if (
        uargs
        and len(uargs) == 1
        and isinstance(uargs[0], string_types)
...
def _url_for_flask(*args, **kw):
    ...
    if (
        args
        and args[0].startswith("api.")
        and isinstance(kw.get("ver"), string_types)
        and kw["ver"].startswith("/")
    ):
            ...
            if kw:
                ...
                for key, val in kw.items():
                    if isinstance(val, (list, tuple)):
                        for value in val:
                            query_args.append("{}={}".format(quote(key), quote(value)))
                    else:
                        query_args.append("{}={}".format(quote(key), quote(val)))
    ...
    if external:
        ...
        my_url = urlunparse(
            (protocol, host, parts.path, parts.params, parts.query, parts.fragment)
        )
...
def _url_for_pylons(*args, **kw):
    ...
    if kw.get("controller") == "api" and kw.get("ver"):
        if isinstance(kw["ver"], int) or not kw["ver"].startswith("/"):
            kw["ver"] = "/%s" % kw["ver"]
...
def url_for_static_or_external(*args, **kw):
    ...
    def fix_arg(arg):
        ...
        url_is_relative = (
            url.scheme == "" and url.netloc == "" and not url.path.startswith("/")
        )
        if url_is_relative:
...
def _local_url(url_to_amend, **kw):
    ...
    if kw.get("qualified", False) or kw.get("_external", False):
        ...
        parts = urlparse(_flask_default_url_for("home.index", _external=True))

        path = parts.path.rstrip("/")
        root = urlunparse(
            (protocol, host, path, parts.params, parts.query, parts.fragment)
        )
...
class _Flash(object):
    ...
    def __call__(
        self, message, category=None, ignore_duplicate=False, allow_html=False
    ):
        if not category:
...
def _link_active_pylons(kwargs):
    highlight_actions = kwargs.get(
        "highlight_actions", kwargs.get("action", "")
    ).split()
    return c.controller == kwargs.get("controller") and c.action in highlight_actions


def _link_active_flask(kwargs):
    blueprint, endpoint = p.toolkit.get_endpoint()
    return kwargs.get("controller") == blueprint and kwargs.get("action") == endpoint
...
def _link_to(text, *args, **kwargs):
    ...
    return link_to(_create_link_text(text, **kwargs), url_for(*args, **kwargs), cls=cls)
...
def _preprocess_dom_attrs(attrs):
    ...
    Python.
    """
    return {key.rstrip("_"): value for key, value in attrs.items() if value is not None}
...
def file(name, value=None, id=None, **attrs):
    ...
    """
    return literal(_input_tag("file", name, value, id, **attrs))
...
def nav_link_pylons(text, *args, **kwargs):
    ...
    if args:
        ...
        log.warning(
            "h.nav_link() please supply controller as a named "
...
def map_pylons_to_flask_route_name(menu_item):
    ...
    if menu_item in LEGACY_ROUTE_NAMES:
        log.info(
            'Route name "{}" is deprecated and will be removed. '
...
def _make_menu_item(menu_item, title, **kw):
    ...
    for need in needed:
        if need not in kw:
            raise Exception("menu item `%s` need parameter `%s`" % (menu_item, need))
...
def get_facet_items_dict(facet, search_facets=None, limit=None, exclude_active=False):
    ...
    if not search_facets or not search_facets.get(facet, {}).get("items"):
        return []
    facets = []
...
def linked_user(user, maxlength=0, avatar=20):
    ...
    if user:
        ...
        return literal(
            "{icon} {link}".format(
                icon=user_image(user.id, size=avatar),
                link=link_to(displayname, url_for("user.read", id=name)),
            )
        )
...
def markdown_extract(text, extract_length=190):
    ...
    return literal(
        text_type(
            truncate(plain, length=extract_length, indicator="...", whole_word=True)
        )
    )
...
def icon_html(url, alt=None, inline=True):
    ...
    if inline:
        classes += "inline-icon "
    return literal(
        ('<img src="%s" height="16px" width="16px" alt="%s" ' + 'class="%s" /> ')
        % (url, alt, classes)
    )
...
def gravatar(email_hash, size=100, default=None):
    ...
    return literal(
        """<img src="//gravatar.com/avatar/%s?s=%d&amp;d=%s"
...
def get_page_number(params, key="page", default=1):
    ...
    try:
        ...
        base.abort(
            400, ('"{key}" parameter must be a positive integer'.format(key=key))
        )
...
@core_helper
def render_datetime(datetime_, date_format=None, with_hours=False, with_seconds=False):
    ...
    if date_format:
        ...
        if datetime_.year < 1900:
            ...
            date_format = re.sub(
                "(?<!%)((%%)*)%y", r"\g<1>{year}".format(year=year[-2:]), date_format
            )
            date_format = re.sub(
                "(?<!%)((%%)*)%Y", r"\g<1>{year}".format(year=year), date_format
            )
    ...
    return formatters.localised_nice_date(
        datetime_, show_date=True, with_hours=with_hours, with_seconds=with_seconds
    )
...
def date_str_to_datetime(date_str):
    ...
    if len(time_tuple) >= 6:
        m = re.match(r"(?P<seconds>\d{2})(\.(?P<microseconds>\d+))?$", time_tuple[5])
        if not m:
            raise ValueError(
...
def dataset_display_name(package_or_package_dict):
    if isinstance(package_or_package_dict, dict):
        return (
            ...
            or package_or_package_dict["name"]
        )
    else:
...
def dataset_link(package_or_package_dict):
    ...
    return link_to(text, url_for("{}.read".format(type_), id=name))
...
@core_helper
def add_url_param(
    alternative_url=None, controller=None, action=None, extras=None, new_params=None
):
    """
    ...
    params_nopage = [(k, v) for k, v in params_items if k != "page"]
    if new_params:
        params_nopage += list(new_params.items())
    if alternative_url:
        return _url_with_params(alternative_url, params_nopage)
    return _create_url_with_params(
        params=params_nopage, controller=controller, action=action, extras=extras
    )
...
def remove_url_param(
    ...
    return _create_url_with_params(
        params=params, controller=controller, action=action, extras=extras
    )
...
@core_helper
def organizations_available(permission="manage_group", include_dataset_count=False):
...
def dashboard_activity_stream(user_id, filter_type=None, filter_id=None, offset=0):
    ...
    if filter_type:
        ...
        return logic.get_action("dashboard_activity_list")(context, {"offset": offset})
...
def format_resource_items(items):
    ...
    for key, value in items:
        ...
        elif (isinstance(value, int) or isinstance(value, float)) and value not in (
            True,
            False,
        ):
...
def resource_preview(resource, package):
    ...
    if datapreview.get_preview_plugin(data_dict, return_first=True):
        url = url_for(
            "{}_resource.datapreview".format(package["type"]),
...
def get_allowed_view_types(resource, package):
    ...
    for plugin in plugins:
        info = plugin.info()
        allowed_view_types.append(
            (info["name"], info.get("title", info["name"]), info.get("icon", "image"))
        )
...
def get_site_statistics():
    stats = {}
    stats["dataset_count"] = logic.get_action("package_search")({}, {"rows": 1})[
        "count"
    ]
    ...
    if not _RESOURCE_FORMATS:
        ...
        with open(format_file_path) as format_file:
            try:
                ...
                raise ValueError(
                    "Invalid JSON syntax in %s: %s" % (format_file_path, e)
                )
            ...
            for format_line in file_resource_formats:
                ...
                for item in line + alternatives:
                    if item:
                        item = item.lower()
                        if (
                            item in _RESOURCE_FORMATS
                            and _RESOURCE_FORMATS[item] != line
                        ):
                            raise ValueError(
                                "Duplicate resource format "
                                "identifier in %s: %s" % (format_file_path, item)
                            )
...
def mail_to(email_address, name):
    ...
    author = escape(name)
    html = Markup(
        '<a href=mailto:{0} style="word-wrap: break-word">{1}</a>'.format(email, author)
    )
    return html
...
def radio(selected, id, checked):
    if checked:
        return literal(
            (
                '<input checked="checked" id="%s_%s" name="%s" \
            value="%s" type="radio">'
            )
...
def get_collaborators(package_id):
    ...
    _collaborators = logic.get_action("package_collaborator_list")(context, data_dict)

    collaborators = []

    for collaborator in _collaborators:
        collaborators.append((collaborator["user_id"], collaborator["capacity"]))
